...
const { readSheet, appendRow, updateRow, findRowByValue } = require('./sheets');
...
/**
 * Migrasi data dari Google Sheets ke Firestore (fungsi ini tidak ada di Code.gs;
 * dibuat agar tombol "Migrasi Data" di UI berfungsi):
 *   - Master_Data       -> koleksi pendaftaran/{ID_Pendaftaran}
 *   - Siswa_Lanjutan    -> koleksi masterSiswa/{ID_Siswa}
 *   - Status_Lanjutan   -> koleksi siswaLanjutan/{ID_Siswa}
 */
async function migrateDataToFirebase() {
  try {
    // 1. Master_Data -> pendaftaran
    const master = await readSheet('Master_Data');
    ...
    for (const row of master.rows) {
      ...
      if (!id) continue;
      const doc = {};
      master.headers.forEach((header, i) => {
        if (header === 'Timestamp') {
    ...
    // 2. Siswa_Lanjutan -> masterSiswa
    const siswa = await readSheet('Siswa_Lanjutan');
    let masterSiswaCount = 0;
    for (const row of siswa.rows) {
      const docIdRaw = row[siswa.headers.indexOf('ID_Siswa')];
      if (!docIdRaw) continue;
      const id = cleanDocId(docIdRaw);
      if (!id) continue;
      await setDoc('masterSiswa', id, {
        Nama_Siswa: row[siswa.headers.indexOf('Nama_Lengkap')],
        Jenjang: row[siswa.headers.indexOf('Kelas_Asal')]
      }, { merge: true });
      masterSiswaCount++;
    }

    // 3. Status_Lanjutan -> siswaLanjutan
    const status = await readSheet('Status_Lanjutan');
    let siswaLanjutanCount = 0;
    for (const row of status.rows) {
      const id = cleanDocId(row[status.headers.indexOf('ID_Siswa')]);
      if (!id) continue;
      const get = (h) => row[status.headers.indexOf(h)];
      const statusVal = get('Status_Kelanjutan');
      const doc = {
        Timestamp: parseDateCell(get('Timestamp')) || new Date(),
        ID_Siswa: id,
        Nama_Siswa: get('Nama_Siswa'),
        Kelas_Asal: get('Kelas_Asal'),
        kelasAsal: get('Kelas_Asal'),
        Kategori_Siswa: get('Kategori_Siswa'),
        kategoriSiswa: get('Kategori_Siswa'),
        Konfirmasi_Kelanjutan: get('Konfirmasi_Kelanjutan'),
        konfirmasi: get('Konfirmasi_Kelanjutan'),
        Status_Kelanjutan: statusVal,
        status: statusVal,
        Alasan: get('Alasan'),
        alasan: get('Alasan'),
        ID_Pendaftaran_Baru: get('ID_Pendaftaran_Baru')
      };
    ...
    return {
      success: true,
      message: `Migrasi selesai: ${pendaftaranCount} pendaftaran, ${masterSiswaCount} siswa master, ${siswaLanjutanCount} status lanjutan di-upload ke Firestore.`
    };
  } catch (e) {

...
const { setDoc, listDocs, patchDocByName, commitWrites, toFirestoreFields } = require('./firestore');
...
/** Generate kode voucher untuk pembukuan stok (server-side fallback). */
function generateVoucherCodeServer(benefitStr) {
  if (!benefitStr || benefitStr === 'None') return '';
  const map = { '1 Stel Seragam': 'S', 'Merchandise': 'M', 'Others': 'O' };
  const suffix = String(benefitStr).split(',').map(s => map[s.trim()] || 'X').join('');
  const ymd = new Date().toISOString().slice(0,10).replace(/-/g,'');
  const rand = Math.random().toString(36).substring(2,6).toUpperCase();
  return `VCH-${ymd}-${rand}-${suffix}`;
}
...
async function backupDataKeSheet(formObj, firebaseId) {
  try {
    // Fallback voucher jika benefit ada tapi kode kosong
    if (formObj.benefitTambahan && formObj.benefitTambahan !== 'None' && !formObj.voucherCode) {
      formObj.voucherCode = generateVoucherCodeServer(formObj.benefitTambahan);
    }
    if (!formObj.benefitTambahan) formObj.benefitTambahan = 'None';
    if (formObj.benefitTambahan === 'None') formObj.voucherCode = '';
    const rowData = [
...
async function handleNewRegistration(formObj, firebaseId) {
  ...
  if (subject) {
    ...
    const benefitText = formObj.benefitTambahan && formObj.benefitTambahan !== 'None'
      ? `<li><strong>Benefit:</strong> ${formObj.benefitTambahan}</li>` : '';
    const voucherText = formObj.voucherCode
      ? `<li><strong>Voucher:</strong> <code>${formObj.voucherCode}</code></li>` : '';
    const htmlBody = `
...
/** Susun array 32 kolom Master_Data (30 asli + 2 Benefit/Voucher). */
...
async function simpanStatusLanjutan(formObj) {
  try {
    ...
    if (formObj.status === 'Lanjut') {
      ...
      // Sinkron ke Firestore agar Rekapitulasi langsung menampilkan siswa lanjutan
      // (sebelumnya hanya Sheet yang ditulis sehingga pendaftaran Lanjutan
      //  tidak muncul di tab Rekap sampai migrasi manual dijalankan).
      try {
        await setDoc('pendaftaran', idPendaftaranBaru, {
          kategori: 'Lanjutan Internal',
          nama: namaSiswa,
          nama_lowercase: String(namaSiswa).toLowerCase(),
          kategoriSiswa: formObj.kategoriSiswa || 'Reguler',
          punyaSibling: 'Tidak',
          namaSibling: '',
          kelasSibling: '',
          jenjang: jenjangTujuan,
          kelas: kelasTujuan,
          periodeSSP: String(periode || ''),
          asalSekolah: 'Madania (Internal)',
          asalInfo: 'Internal',
          noFormulir: formObj.noFormulir || '',
          tglFormulir: formObj.tglFormulir || '',
          statusBooking: formObj.statusBooking || 'Tidak',
          tglBooking: formObj.tglBooking || '',
          metodeBayar: formObj.metodeBayar || '',
          Status_Pendaftaran: 'Aktif',
          Tarif_SSP: String(tarifSSP),
          Diskon: String(diskon),
          Tarif_Nett: String(tarifNett),
          termin1: String(t1), tglT1: formObj.tglT1 || '',
          termin2: String(t2), tglT2: formObj.tglT2 || '',
          termin3: String(t3), tglT3: formObj.tglT3 || '',
          Total_Bayar: String(totalBayar),
          Persentase_Bayar: String(persen),
          benefitTambahan: formObj.benefitTambahan || 'None',
          voucherCode: formObj.voucherCode || '',
          Timestamp: timestamp
        }, { merge: true });
        await setDoc('siswaLanjutan', String(idSiswa), {
          ID_Pendaftaran_Baru: idPendaftaranBaru
        }, { merge: true });
      } catch (fsErr) {
        console.error('Gagal sinkron lanjutan ke Firestore: ' + (fsErr.message || fsErr));
      }
...
const MASTER_TO_FIRESTORE = {
  ...
  'Benefit_Tambahan': 'benefitTambahan',
...
async function updateSearchField(collection, nameField, lowercaseField, pageToken) {
  try {
    // pageSize kecil (30) agar respons & jumlah write per panggilan tetap ringan
    // dan muat dalam batas waktu serverless Vercel (10 detik di paket Hobby).
    const { documents, nextPageToken } = await listDocs(collection, { pageSize: 30, pageToken: pageToken || null });

    if (!documents || documents.length === 0) {
      return { success: true, processedCount: 0, nextPageToken: null };
    }

    // Kumpulkan semua update lalu kirim dalam SATU panggilan Firestore (:commit),
    // bukan satu panggilan per dokumen — agar setiap panggilan API selesai jauh
    // di bawah batas waktu serverless Vercel (10 detik di paket Hobby).
    const writes = [];
    let updatedCount = 0;
    for (const doc of documents) {
      ...
      if (data && data[nameField] && data[nameField].stringValue && !data[lowercaseField]) {
        writes.push({
          update: {
            name: doc.name,
            fields: toFirestoreFields({ [lowercaseField]: data[nameField].stringValue.toLowerCase() })
          },
          // updateMask harus SAUDARA dari update (bukan di dalamnya) sesuai
          // REST API Firestore: writes[].updateMask — bukan writes[].update.updateMask.
          updateMask: { fieldPaths: [lowercaseField] }
        });
        updatedCount++;
      }
    }

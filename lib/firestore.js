...
/**
 * Menulis banyak dokumen dalam SATU panggilan Firestore (:commit).
 * @param {object[]} writes Array berisi { update: { name, fields }, updateMask }.
 *   updateMask adalah SAUDARA dari update (writes[].updateMask), bukan
 *   bagian dari objek update — sesuai REST API Firestore :commit.
 */
async function commitWrites(writes) {
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => '');
    throw new Error(`Firestore commit error (${res.status}): ${text}`);
  }
  return res.json();
}

module.exports = { getDoc, setDoc, listDocs, patchDocByName, commitWrites, toFirestoreFields, fromFirestoreFields };
